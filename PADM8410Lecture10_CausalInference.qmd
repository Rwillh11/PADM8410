...
We observe the potential outcome that matches your actual treatment status. If you were treated ($D_i = 1$), we see $Y_i(1)$. If not ($D_i = 0$), we see $Y_i(0)$. The other potential outcome is hidden from us.
:::
...
Among the people who actually got treated, how much did the treatment help them on average? This is often different from the ATE, because the people who get treated aren't a random sample. Sicker patients get surgery, motivated students seek tutoring, etc.
...
-   **Heterogeneous Effect Bias**: The treatment works differently for different people, and the people who get treated aren't representative
    -   Surgery might be most beneficial for the sickest patients, but those are exactly the ones who got it

-   Bottom line: naive comparisons can be wildly misleading when treatment isn't random
:::
...
When is this realistic? **Randomized experiments.** That's basically it. Everything else requires more assumptions.
...
-   In causal studies, we're often not trying to generalize to a larger population
    ...
    -   The core uncertainty is not about sampling, it's about the fact that **we don't know the counterfactual**
...
-   **Randomization inference** generates p-values directly from the randomization itself
    ...
    -   The logic is similar to the bootstrap
:::
...
::: incremental
-   Works when treatment is truly randomized and you want exact, assumption-free p-values
...
We use $U$ for variables we can't measure but believe exist. These are often the biggest headache for identification.
...
-   The indirect effect is often called the **Average Causal Mediation Effect (ACME)**: how much of the treatment's effect operates through the mediator?
:::
...
::: incremental
1.  Counterfactuals
2.  Potential outcomes ($Y(1)$, $Y(0)$, ATE)
3.  Selection bias
4.  Randomization inference
5.  Estimands
6.  DAGs (chains, forks, colliders)
7.  Identification
8.  Mediation
:::
